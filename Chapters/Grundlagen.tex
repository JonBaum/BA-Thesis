...
Ähnlich wie in Iturriaga et al. \cite{Iturriaga.2017}, Pinzon et al. \cite{Pinzon.23.04.201726.04.2017} und Risbeck et al. \cite{Risbeck.2017} erfolgt eine Linearisierung der nicht-linearen Gleichungen, sodass es sich um ein MILP handelt.
...
Bei diesen handelt es sich um den Standard-Zustand des parametrisierten Gebäudes im Bestand, einer normalen energetischen Nachrüstung und außerdem einer fortgeschrittenen.
...
Somit kann die Emission beschrieben werden als Summe der CO\(_2\)-Freisetzung, die bei Nutzung der Brennstoffe frei werden.
Hierbei ist noch zu beachten, dass sich die Emission durch Gutschriften aus Stromeinspeisungen in das Netz reduzieren.
...
Neben den Zielfunktionen werden die Möglichkeiten der verschiedenen Lösungen durch Restriktionen eingeschränkt.
So wird beispielsweise die Anzahl der PV-Module durch die zur Verfügung stehende Dachfläche eingeschränkt.
Eine zentrale Nebenbedingung stellt die Berechnung der Heizlast dar.
Als Grundlage dient die DIN EN 12831.
Diese definiert die Norm-Heizlast \(\Phi_{HL, build}\) als Summe der Transmissionswärmeverlusten \(\Phi_T\) und Lüftungswärmeverlusten \(\Phi_{V, build}\) abzüglich der Aufheizleistung \(\Phi_{hu}\) und der internen Gewinne \(\Phi_{gain}\).
...
Die Transmissionswärmeverluste \(\Phi_T\) lassen sich als Produkt der Temperaturdifferenz zwischen der Norm-Innentemperatur (\(\Theta_{int, i}\)) und der Außentemperatur (\(\Theta_e\)) mit dem Transmissionswärmetransferkoeffizienten (\(H_t\)).
Letzterer lässt sich ausdrücken als die Summe über die Bauteile des Produktes aus Bauteilfläche (\(A_k\)), U-Wert (\(U_k\)) und Temperaturkorrekturfaktor (\(f_{U,k}\)).
...
\Phi_T = H_t * (\Theta_{int, i} - \Theta_e)
\end{equation}
\begin{equation}
\label{eq:Gleichung2625}
H_t = \sum_{k} A_k * (U_k + \Delta U_{TB}) * f_{U,k}
\end{equation}
Hierbei wird \(\Delta U_{TB}\) laut Norm vereinfacht mit \(\Delta U_{TB} = 0,05 W/(m^2*K)\) angenommen.
Der Temperaturkorrekturfaktor berücksichtigt einen Unterschied zwischen Norm-Innentemperatur und Außentemperatur.
Da in dem Programm keine Unterteilung keine Unterscheidung der Räume geschieht, gibt es nur im Falle des Bodens aufgrund des Kontaktes mit dem Erdreich einen solchen Unterschied.
Daher wird \(f_{U, Boden}\) mit 0,6 angenommen.
