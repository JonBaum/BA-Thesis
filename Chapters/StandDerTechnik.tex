...
Harb et al. & MILP & \begin{tabular}[c]{@{}l@{}}Gebäude/\\ Quartier\\Auslegung\end{tabular} & \begin{tabular}[c]{@{}l@{}}Mikro-BHKW, WP,\\ Elektroheizstäbe, PV, \\ thermische ES, Kessel, \\ Nahwärmenetz\end{tabular} & Jährliche Kosten \\ \hline
\rowcolor[HTML]{EFEFEF} 
...
Hierzu wird laut Norm von einer Luftwechselrate für ein nicht-luftdichtheitsgeprüftes Gebäude von n = 0,7\, ausgegangen. 
...
Man unterscheidet hierbei zwischen maschineller und natürlicher Lüftung, welche durch Temperatur- und Windeinwirkung hervorgerufen wird \cite{Maas.2017}.
...
\end{equation}
Der dimensionslose Parameter C\(_d\) wird durch Messungen bestimmt und nimmt Werte zwischen 0,39 und 0,65 an. 
...
Mit diesem ergibt sich q\(_{V,open,th}\) zu
\begin{equation}
...
DIN EN 12831 & \(A_{eff} \cdot 3600 \cdot \sqrt{\frac{C_{D}^2 \cdot g \cdot h_m \cdot \Delta \theta}{9 \cdot \theta_e} + 0,05 \cdot v_{fac}}\) & \begin{tabular}[c]{@{}l@{}}XXXX \\ XXXX\end{tabular} \\ \hline
\end{tabular}
...
Neben den Wärmeverlusten können bei einem hohen Infiltrationsluftwechsel auch Zugerscheinungen und somit Unbehaglichkeiten der Bewohner auftreten. 
Weiterhin kann es durch das Auskondensieren der Luftfeucht beim Übergang durch die Gebäudehülle zu bauphysikalischen Schäden kommen. \cite{Schild.2013}
...
Allerdings kann in diesem Fall keine absolute Öffnungsfläche benannt werden, welche an der Infiltration beteiligt ist.
...
Somit wird eine Luftwechselrate bei 50\,Pa Überdruck (n\(_{50}\)) bestimmt. \cite{Schild.2013} \\
Dieser Kennwert gibt Aufschluss über die Dichtheit des Gebäudes und wird durch die DIN 4108-7 von einem wärmeschutztechnischen Blickpunkt aus für Neubauten und energetisch sanierte Albauten reglementiert. 
Hierbei wird zwischen Gebäuden mit und ohne istallierter raumlufttechnischer Anlage (RLT) unterschieden.
Tabelle \ref{tab: Tabelle3121} gibt die maximalen Norm-Werte n\(_{50, max}\) wieder.
...
Die minimalen Norm-Werte n\(_{50, min}\) sind für verschiedene Gebäudeausführungen in Tabelle \ref{tab: Tabelle3121} aufgeführt.
...
\label{tab: Tabelle3121}
\end{table}

Im Gegensatz zu der Lüftung durch Fenster kann aufgrund der schwierigen nummerischen Festlegung der Dichtheit eines Bestandsgebäudes keine analytische Lösung zur Bestimmung des Infiltrationsluftvolumenstroms herangezogen werden.
